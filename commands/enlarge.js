...
const fs = require('fs');
const emoteList = require("./json/emoji.json");
...
async execute(client, arguments, receivedMessage) {
        ...
        if(arguments.join(" ").toLowerCase().includes("-nn")) resizeMode = {method: Jimp.RESIZE_NEAREST_NEIGHBOR, name: "Nearest Neighbour"}
        if(arguments.join(" ").toLowerCase().includes("-bl")) resizeMode = {method: Jimp.RESIZE_BILINEAR, name: "Bilinear"}
        if(arguments.join(" ").toLowerCase().includes("-bc")) resizeMode = {method: Jimp.RESIZE_BICUBIC, name: "Bicubic"}
        if(arguments.join(" ").toLowerCase().includes("-he")) resizeMode = {method: Jimp.RESIZE_HERMITE, name: "Hermite"}
        if(arguments.join(" ").toLowerCase().includes("-be")) resizeMode = {method: Jimp.RESIZE_BEZIER, name: "Bezier"}
        ...
        var dimensionsArgs = (" " + arguments.join(" ")).match(/([x ]\d*)/gi)
        if(dimensionsArgs) {
            while(dimensionsArgs[0] == " ") {
                dimensionsArgs.shift()
            }
        }
        var factorArg = arguments.join(" ").match(/\b(f\d*)/gi)
        ...
        if(dimensionsArgs && dimensionsArgs[0] !== " " && dimensionsArgs.length > 0) {
            ...
            if(isNaN(dim1)||isNaN(dim2)) return receivedMessage.reply(`Invalid resize resolution`)
            useFactor = false
        } else if(factorArg) {
            var f = parseInt(factorArg[0].replace(/[^\d]/gi, ""))
            if(isNaN(f)||f > 10||Math.sign(f) < 0) return receivedMessage.reply(`Invalid factor size`)
        } else {
            var f = 1
        }

        var customMatch = arguments.join(" ").match(/(?<=<a?:.*:)\d*(?=>)/gi)
        var uniMatch = receivedMessage.cleanContent.match(/[\u{1f300}-\u{1f5ff}\u{1f900}-\u{1f9ff}\u{1f600}-\u{1f64f}\u{1f680}-\u{1f6ff}\u{2600}-\u{26ff}\u{2700}-\u{27bf}\u{1f1e6}-\u{1f1ff}\u{1f191}-\u{1f251}\u{1f004}\u{1f0cf}\u{1f170}-\u{1f171}\u{1f17e}-\u{1f17f}\u{1f18e}\u{3030}\u{2b50}\u{2b55}\u{2934}-\u{2935}\u{2b05}-\u{2b07}\u{2b1b}-\u{2b1c}\u{3297}\u{3299}\u{303d}\u{00a9}\u{00ae}\u{2122}\u{23f3}\u{24c2}\u{23e9}-\u{23ef}\u{25b6}\u{23f8}-\u{23fa}]/ug)
        ...
        if(customMatch) {
            customMatch.forEach(async cm => {
                ...
                .catch(async err => {
                    ...
                    .catch(err => {
                        return receivedMessage.reply('Couldn`t resolve the image from your Emote.')
                    });
                });
            })
        } 
       ...
       if(eURL) {
        await Jimp.read(eURL)
                .then(image => {
                    ...
                    return receivedMessage.reply('Couldn`t resolve the image from your Attachment.')
                })
        } 
        if(linkArg) {
            await Jimp.read(linkArg[0])
                .then(image => {
                    ...
                    return receivedMessage.reply('Couldn`t resolve the image from your Link.')
                })
        }
        setTimeout(() => {
            setTimeout(() => {
                if(JimpArray.length > 1) {
                    ...
                    .then((image) => {
                        image.write(`./images/merge${operationid}.png`, (err) => {
                            ...
                            if(useFactor){resizeEmbed.setAuthor(`Image scaled`, receivedMessage.author.avatarURL)}else resizeEmbed.setAuthor(`Image resized`, receivedMessage.author.avatarURL)
                            ...
                            receivedMessage.channel.send({embed: resizeEmbed}).then(() => {
                ...
                } else return receivedMessage.reply(`You must specify 1 or more emotes or images or links`)
            }, lal + uml + cml * 1000);
        }, 1000);
    }
};