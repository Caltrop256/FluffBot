...
module.exports = {
   ...
   execute(client, arguments, receivedMessage) {
    ...
    const waveString = (string) => {
        ...
        if(arguments.length) {
            let uwu = waveString(arguments.join(" "));
            let uwuName = waveString(receivedMessage.member.displayName)

            var UwUEmbed = new Discord.RichEmbed()
                    .setColor(embedNeon_Pink)
                    .setAuthor(waveString(uwuName), receivedMessage.member.user.avatarURL)
                    .setDescription(uwu)
        ...
        if(!arguments.length) {
            receivedMessage.channel.fetchMessages({ limit: 20 }).then(messages => {
                ...
                var UwUEmbed = new Discord.RichEmbed()
                    ...
                    .setDescription(uwu)
                    .setFooter(waveString(`requested by ${receivedMessage.member.displayName}`), receivedMessage.member.user.avatarURL)
