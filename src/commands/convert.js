...
var convert = require('./cvUnits/index.js')
...
module.exports = {
    ...
    execute(client, arguments, receivedMessage) {
            ...
            if(!arguments.length) {
                var allUnits = new Discord.RichEmbed()
                .setAuthor(`Available Units`, receivedMessage.author.displayAvatarURL)
                .setTimestamp()
                .setColor(embedGreen)
                ...
                return receivedMessage.channel.send({embed: allUnits})
            }
            ...
            var sepArg1 = arguments[0].match(/[a-z/²³]+|[^a-z]+/gi)
            ...
            if(arguments.length >= 3) {
                var convAmt = parseInt(arguments[0])
                var convFrom = findUnit(arguments[1])
                var convTo = findUnit(arguments[2])
            } else {
                var failSafe = sepArg1[0] ? findUnit(sepArg1[0]) : null
                var convAmt = sepArg1[0] ? parseInt(sepArg1[0]) : null
                var convTo = arguments[1] ? findUnit(arguments[1]) : null
                var convFrom = sepArg1[1] ? findUnit(sepArg1[1]) : null
            }

            if((!convFrom || !convTo) && !failSafe) return receivedMessage.reply(`I don't recognise those units, sorry.`)
            ...
            if((!convTo || isNaN(convAmt) || (!sepArg1[1] && !convFrom)) && (failSafe || convFrom)) {
                ...
                return receivedMessage.channel.send("You didn't provide any output units.\n" + `\`${failSafe.abbr.replace(/2/gi, "²").replace(/3/gi, "³")}\` can be converted to ${clean.replace(/2/gi, "²").replace(/3/gi, "³")}`)
            }
            if(convFrom.measure != convTo.measure) return receivedMessage.reply(`You can't convert between \`${convFrom.measure}\` and \`${convTo.measure}\`!`)
            ...
            var ConversionEmbed = new Discord.RichEmbed()
            .setAuthor("Unit Conversion", receivedMessage.member.user.avatarURL, receivedMessage.member.user.avatarURL)
            .setColor(embedGreen)
            .addField("Input", `\`${sepArg1[0]}\` ${convertedFromUnitClean}`, true)
            .addField("Output", `\`${conversion}\` ${convertedToUnitClean}`, true)
            .setFooter(`Converted in: ${hrend[0]} seconds, ${prettyMs(hrend[1] / 1000000, {formatSubMilliseconds: true, verbose: true})}`)

            receivedMessage.channel.send({embed: ConversionEmbed})
    }
}